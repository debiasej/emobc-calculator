...
public class ParseUtils {
	...
	/*-------------CALCULATOR CONSTANS---------------------------------------*/
	private static final String _NUM1_TAG_ = "num1";
	private static final String _NUM2_TAG_ = "num2";
...
//			return parseListLevelDataFile(xpp);
		...
		case CALCULATOR_ACTIVITY:
			return parseCalculatorLevelDataFile(xpp);
		default:
			break;
		}
	...
	/**
	 * Generate a table of the elements of XmlPullParser useful for the Calculator
	 * @param xpp
	 * @return
	 */
	private static Map<String, Object> parseCalculatorLevelDataFile(XmlPullParser xpp) {
		final Map<String, Object> ret = new HashMap<String, Object>();
		...
		NwXmlStandarParser parser = new NwXmlStandarParser(xpp,
				new NwXmlStandarParserTextHandler() {
					...
					//private List<AppButton> currList;
					//private AppButton currButton;
					private CalculatorLevelDataItem currItem;
					private NextLevel nextLevel;
					...
					public void handleText(String currentField, String text) {
						if(currentField.equals(_DATA_TAG_)){
							currItem = new CalculatorLevelDataItem();
							appLevelData.addItem(currItem);	
						}else if(currentField.equals(_DATA_ID_TAG_)){
							currItem.setId(text);
						}else if(currentField.equals(_HEADER_IMAGE_FILE_TAG_)){
							...
							currItem.setHeaderText(text);
						}else if(currentField.equals(_NUM1_TAG_)){
							currItem.setNumber1(Integer.parseInt(text));
						}else if(currentField.equals(_NUM2_TAG_)){
							currItem.setNumber2(Integer.parseInt(text));
						}else if(currentField.equals(_NEXT_LEVEL_TAG_)){
							nextLevel = new NextLevel();
							currItem.setNextLevel(nextLevel);
						}else if(currentField.equals(_NL_LEVEL_ID_TAG_)){
